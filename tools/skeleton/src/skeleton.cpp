...
/********************************************************************
 ...
 Copyright (C) 2005--2018 N.Yu.Zolotykh
...
void print_help()
{
  ...
  std::cout << "Out.options:{--outputinfile} / --nooutputinfile \n";
  ...
  std::cout << "Out.options:{--columnsequalwidth} / --aligncolumns / --nocolumnalignment  \n";
...
int Skeleton::main(int argc, char *argv[])
{
  ...
  int graphadj = 1;
  int matrixoutputformat = COLUMNSEQUALWIDTH;
  ...
  for(int i = 1; i < argc; i++)
  {
    ...
    else if (strcmp(argv[i], "--nomatrices") == 0)
    {
      ...
      modification = DDM_MIN_PAIRS;
    else if(strcmp(argv[i], "--maxpairs") == 0)
      modification = DDM_MAX_PAIRS;
      ...
      inputfromstdin = 0;
    else if(strcmp(argv[i], "--columnsequalwidth") == 0)
      matrixoutputformat = COLUMNSEQUALWIDTH;
    else if(strcmp(argv[i], "--aligncolumns") == 0)
      matrixoutputformat = ALIGNCOLUMNS;
    else if(strcmp(argv[i], "--nocolumnalignment") == 0)
      matrixoutputformat = NOCOLUMNALIGNMENT;
  ...
  if (modification == DDM_MIN_CUT_OFF || modification == DDM_MAX_CUT_OFF 
    ||  modification == DDM_MIN_PAIRS || modification == DDM_MAX_PAIRS)
  {
    if (prefixed_order == 1)
