...
// Copyright (C) 2005--2013 N.Yu.Zolotykh
...
#define DDM_RANDOM        6
#define DDM_MIN_PAIRS     7
#define DDM_MAX_PAIRS     8
...
/*
double gcd(big_int a, big_int b)
{
  //error
  return 1.0;
}
...
template <class T>
double vgcd(T a) // ��� ��������� ������� �� ���������� ���������� ���� � Arageli (gcd(������))  ����� ��� ���������� ��� ������ vgcd �������� �� gcd
{
...
template <class T>
size_t num_pairs(typename std::list<size_t>::iterator ine, typename std::list<ray<T>*>& rays)
{
  size_t nplus = 0;
  size_t nminus = 0;
...
template <class T>
size_t num_cut_off(typename std::list<size_t>::iterator ine, typename std::list<ray<T>*>& rays)
{
  size_t nminus = 0;
  for (typename std::list<ray<T>*>::iterator v = rays.begin(); v != rays.end(); ++v)
  {
  ...
  typename std::list<size_t>& inequalities, int order)
{
  ...
  switch (order)
  {  
     case DDM_MIN_PAIRS: case DDM_MAX_PAIRS:
       f_selected = num_pairs(ine_selected, rays);
       break;
     case DDM_MIN_CUT_OFF: case DDM_MAX_CUT_OFF:
       f_selected = num_cut_off(ine_selected, rays);
       break;
  }
  ...
  for (; ine != inequalities.end(); ++ine)
  {
    ...
    {  
       case DDM_MIN_PAIRS: case DDM_MAX_PAIRS:
         f = num_pairs(ine, rays);
         break;
       case DDM_MIN_CUT_OFF: case DDM_MAX_CUT_OFF:
         f = num_cut_off(ine, rays);
         break;
    }
    switch (order)
    {  
       case DDM_MIN_PAIRS: case DDM_MIN_CUT_OFF:
         if (f < f_selected)
         {
           f_selected = f;
           ine_selected = ine;
         ...
         break;
       case DDM_MAX_PAIRS: case DDM_MAX_CUT_OFF:
         if (f > f_selected)
         {
           f_selected = f;
           ine_selected = ine;
...
*/
  ...
  for (size_t iine = r; iine < m; iine++)
  {
    ...
    /* 3 types of rays: rays_plus, rays_minus, rays_zero */
    ...
    cur != rays.end(); ++cur)
  {
    if (!intarith)
    {
       T delta = firstnonzero((*cur)->coordinates, eps); // ��� ������� ����� �������, ��� �����, ���� ��������� � ����
       (*cur)->coordinates /= delta;                     // �� ���� ��� �������� � �������, �� ����� �������� ���      
       (*cur)->discrepancy /= delta;                     // (��. 2 ������������������ �����
       clearzeros((*cur)->coordinates, eps);             // ��, ��� ������������� �������� � �������
  ...
  const matrix<int>& dis_int, std::vector<int>& ine_type, matrix<size_t>& ridges)
{
  ...
  for (size_t i = m; i > 0; )
  {
    ...
    if (ine_type[i] == INE_IMPLICIT_EQUATION) 
      continue;
    // ��������: if num_zeros < d_U - d_L - 1 // then redundant
